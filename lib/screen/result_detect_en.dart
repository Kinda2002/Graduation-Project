...
class ResultDetectEn extends StatefulWidget {
  const ResultDetectEn({super.key});

  @override
  State<ResultDetectEn> createState() => _ResultDetectEnState();
}

class _ResultDetectEnState extends State<ResultDetectEn> {
  ...
  void toggleCamera() {
    setState(() {
      isFrontCamera = !isFrontCamera; // تبديل موقع الكاميرا
      initializeCamera(); // إعادة تهيئة الكاميرا بناءً على الموقع الجديد
    });
  }
  ...
  Future<String> sendImageToServer(List<int> imageBytes) async {
    final uri = Uri.parse('http://172.20.10.2:8004/predict/');
